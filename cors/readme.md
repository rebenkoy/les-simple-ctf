...
---
На главной — две ссылки: **«Submit to the reviewer»** (захостить страницу в Design Grader) и
**«My space»** (позвать `/api/account` от своего имени — без сессии ревьюера вернётся только гость,
без `secret_note`). Ревьюеры проверяют присланные макеты в **Design Grader**
([https://grader.{host}](https://grader.{host})), будучи в это время залогинены здесь. Пришли страницу,
которая кросс-доменно прочитает их `/api/account` и отправит `secret_note` тебе — свой личный адрес
возьми во вкладке **Listener**. Хостить ничего не надо: грейдер сам покажет ревьюеру твою страницу.
